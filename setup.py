...
class CMakeBuild(build_ext):
    def build_extension(self, ext):
        ...
        cmake_args = [
            ...
            "-DLIBRAOP_VERSION_INFO={}".format(self.distribution.get_version()),
            "-DCMAKE_BUILD_TYPE={}".format(cfg),  # not used on MSVC, but no harm
        ]

        # Change the open_ssl root dir on mac os if it was installed with homebrew
        brew_ssl_path = "/usr/local/opt/openssl"
        if os.path.isdir(brew_ssl_path):
            cmake_args = ["-DOPENSSL_ROOT_DIR={}".format(brew_ssl_path)] + cmake_args
